...
docker save airline-api:demo | gzip | ssh "${SSH_TARGET}" 'gunzip | docker load'
printf 'DATABASE_URL=%s\n' "${DB_URL}" | ssh "${SSH_TARGET}" 'umask 077; mkdir -p /opt/airline; cat > /opt/airline/runtime.env'
ssh "${SSH_TARGET}" "docker rm -f airline-api 2>/dev/null || true; docker run -d --name airline-api --restart unless-stopped -p 8000:8000 --env-file /opt/airline/runtime.env airline-api:demo"
ssh "${SSH_TARGET}" "docker exec airline-api python -m airline_core.persistence.bootstrap --with-demo-data"
