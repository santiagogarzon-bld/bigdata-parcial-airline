...
| FR-036 | The ETL shall load approved revenue, refunds, and voluntary cancellations from OLTP into PostgreSQL OLAP. | ETL process | Must | A controlled source dataset reconciles with OLAP counts and monetary totals after a run. | Q-023–Q-024 |
...
| FR-038 | The analyst shall query gross and net revenue by fare/cabin and cancellation patterns by route, date, channel, and advance-purchase band. | Analyst | Must | Seeded scenarios return expected grouped results verified by automated SQL assertions. | Q-023, Q-045, Q-049 |
| FR-039 | The target architecture shall schedule ETL hourly, while Learner Lab shall permit controlled on-demand execution for cost-limited evidence. | ETL process | Must | Architecture documents one-hour freshness; demonstration evidence records each manually triggered run and its data cutoff. | Q-024, Q-026 |
